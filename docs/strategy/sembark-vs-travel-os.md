...
**Defer:** *(closed — see Per-party markup override below)* · *(closed — see Per-line markup presets below)*.

#### Prod-ready ladder — Per-party markup override (**done**)

| Wave | Status | What shipped |
|------|--------|----------------|
| **1 Integrity** | **Done** | `metadataJson.markupPercent` via `UpdatePartySchema.markupPercent`; `resolveOrgMarkupPercent` prefers party override over org agent/default; Match resolve loads party metadata |
| **2 Channels / UI** | **Done** | Customer hub **Markup override %**; hub cue; trip Apply default uses client override label |
| **3 Proof** | **Done** | `orgMarkup.test` + `partyHub.test`; this ladder in memo |

**Defer:** *(closed — see Per-line markup presets + Party markup stamp on send below)*.

#### Prod-ready ladder — Per-line markup presets (**done**)

| Wave | Status | What shipped |
|------|--------|----------------|
| **1 Integrity** | **Done** | `markupDetailsFromPreset` + `markupPresetId` / `markupPresetLabel` on `QuotationItemDetails`; apply from org library onto a single line |
| **2 Channels / UI** | **Done** | Quote service detail **Org presets (this line)** chips for hotel / transfer / activity / custom |
| **3 Proof** | **Done** | `markup-presets.spec` + web `markupPresets.test`; this ladder in memo |

**Defer:** *(closed — see Party markup stamp on send below)*.

#### Prod-ready ladder — Party markup stamp on send (**done**)

| Wave | Status | What shipped |
|------|--------|----------------|
| **1 Integrity** | **Done** | `stampPartyMarkupOntoQuoteItems` freezes `partyMarkupPercent` + `partyMarkupSource` onto each line at quote `send` (email / WhatsApp / mark-sent); idempotent; does not rewrite sell |
| **2 Channels / UI** | **Done** | Service detail **Frozen at send · client markup** cue |
| **3 Proof** | **Done** | `org-markup.spec` + `party-markup-stamp.spec` + web `orgMarkup.test`; this ladder in memo |

**Defer:** rewriting sell from frozen party % on rematch of sent versions (explicitly out — freeze is audit-only).
...
**Defer:** *(closed — see Matrix delete of cleared meals below)* · CSV matrix meal columns.

#### Prod-ready ladder — Matrix delete of cleared meals (**done**)

| Wave | Status | What shipped |
|------|--------|----------------|
| **1 Integrity** | **Done** | `diffMealOccupancyMatrix` emits `deletes` when a sibling meal’s cells are blank; open/anchor tip cannot be cleared via matrix |
...
| Wave | Status | What shipped |
|------|--------|----------------|
| **1 Integrity** | **Done** | `POST …/transfer-fares|activity-rates/:id/restore-field` merges one commercial field from prior tip onto new tip from active; pending/`rates.approve` unchanged |
...
| Wave | Status | What shipped |
|------|--------|----------------|
| **1 Integrity** | **Done** | Pending transfer/activity new-version + restore enqueue high-priority Task (+ notify assignee); Activate marks Task done; audit `taskId` on create |
...
**Defer:** **Parked** — multi-step quorum / `ApprovalRequest` (dual-control Activate + Task inbox already thin-complete); *(field restore — see Transfer/activity tip field-level restore)*.
