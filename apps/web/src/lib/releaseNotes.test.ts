...
describe('releaseNotes', () => {
  ...
  it('keeps recent proven rate-grid notes buyer-visible', () => {
    ...
    expect(top[0]?.id).toBe('2026-07-20-instalment-schedule-from-terms');
    expect(top[1]?.id).toBe('2026-07-20-party-markup-override');
    expect(top[2]?.id).toBe('2026-07-20-credit-limit-gates');
    expect(top[3]?.id).toBe('2026-07-20-markup-preset-library');
    expect(top[4]?.id).toBe('2026-07-20-credit-terms-automation');
    expect(
