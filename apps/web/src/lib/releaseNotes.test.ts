...
describe('releaseNotes', () => {
  ...
  it('keeps recent proven rate-grid notes buyer-visible', () => {
    ...
    expect(top[0]?.id).toBe('2026-07-20-fx-lock-live-fetch');
    expect(top[1]?.id).toBe('2026-07-20-fx-auto-cron');
    expect(top[2]?.id).toBe('2026-07-20-refund-request-approve');
    expect(top[3]?.id).toBe('2026-07-20-package-folder-dnd');
    expect(top[4]?.id).toBe('2026-07-20-auto-extend-min-stay');
    expect(
